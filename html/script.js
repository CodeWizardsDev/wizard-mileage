...
let WEAR_COMPONENTS = {
    sparkPlug: { id: 'sparkPlugWear', label: 'Spark Plugs', icon: 'fas fa-bolt', type: 'engine' },
    ...
    brake: { id: 'brakeWear', label: 'Brakes', icon: 'fas fa-record-vinyl', type: 'brakes' },
    suspension: { id: 'suspensionWear', label: 'Suspension', icon: 'fas fa-car-burst', type: 'suspension' },
...
const HEALTH_STATUS = {
    ...
    disabled: { class: 'health-disabled', label: 'DISABLED' }
};
...
function getDisabledHealthStatus() {
    return { class: 'health-disabled', label: 'DISABLED' };
}
...
function updateProgressBar(barId, value) {
    ...
    if (value === null || typeof value === 'undefined') {
        ...
        const { class: statusClass, label } = getDisabledHealthStatus();
        ...
        vehicleEditForm.addEventListener('submit', function(event) {
            ...
            if (!plate || isNaN(mileage) || mileage < 0) {
                fetch(`https://${GetParentResourceName()}/notify`, {
                  ...
                  body: JSON.stringify({ message: 'Please enter valid plate and mileage.', type: 'error' })
                })
                return;
            }
            const vehicleData = {
            ...
            fetch(`https://${GetParentResourceName()}/updateVehicleData`, {
                ...
                if (data.success) {
                    fetch(`https://${GetParentResourceName()}/notify`, {
                      ...
                      body: JSON.stringify({ message: 'Vehicle data updated successfully', type: 'success' })
                    })
                    closeVehicleEdit();
                    fetch(`https://${GetParentResourceName()}/requestVehicleList`, {
                        method: 'POST'
                    }).then(res => res.json()).then(resData => {
                        window.postMessage({ type: 'vehicleList', vehicles: resData.vehicles }, '*');
                    });
                } else {
                    fetch(`https://${GetParentResourceName()}/notify`, {
                      method: 'POST',
                      ...
                      body: JSON.stringify({ message: 'Failed to update vehicle data: ' + (data.error || 'Unknown error'), type: 'error' })
                    })
                }
            });
        });
        ...
        deleteVehicleBtn.addEventListener('click', function() {
            ...
            if (!plate) {
                fetch(`https://${GetParentResourceName()}/notify`, {
                  ...
                  body: JSON.stringify({ message: 'Invalid Plate', type: 'error' })
                })
                return;
            }
            ...
            fetch(`https://${GetParentResourceName()}/deleteVehicle`, {
                ...
                if (data.success) {
                    fetch(`https://${GetParentResourceName()}/notify`, {
                      ...
                      body: JSON.stringify({ message: 'Vehicle deleted successfully.', type: 'success' })
                    })
                    closeVehicleEdit();
                    fetch(`https://${GetParentResourceName()}/requestVehicleList`, {
                        method: 'POST'
...
window.addEventListener('message', ({ data }) => {
    const handlers = {
        ...
        updateWear: () => {
            ...
            const wearTypes = [
                { key: 'sparkPlugPercentage', id: 'sparkPlugWear' },
                { key: 'oilPercentage', id: 'oilWear' },
                { key: 'filterPercentage', id: 'filterWear' },
                { key: 'airFilterPercentage', id: 'airFilterWear' },
                { key: 'tirePercentage', id: 'tireWear' },
                { key: 'brakePercentage', id: 'brakeWear' },
                { key: 'suspensionPercentage', id: 'suspensionWear' },
                { key: 'clutchPercentage', id: 'clutchWear' }
            ];
...
function openVehicleEdit(vehicle) {
    ...
    // Set all other fields, defaulting to empty string if undefined, format to 2 decimals
      ...
      window.addEventListener('message', function(event) {
        if (event.data.type === 'setPlayerSettings') {
          ...
          function getStatusForValue(value) {
            if (value >= 75) {
              return { class: 'health-excellent', label: 'Excellent' };
            } else if (value >= 50) {
              return { class: 'health-good', label: 'Good' };
            } else if (value >= 25) {
              return { class: 'health-warning', label: 'Warning' };
            } else {
              return { class: 'health-critical pulse', label: 'Critical' };
            }
          }
